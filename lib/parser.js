...
module.exports = {
  ...
  formatPRIVMSG(event) {
    ...
    const content = split_msg_parts.join(' :').replace(/\r\n/g, '');
  ...
  formatCLEARCHAT(event) {
    ...
    if (target_username) parsed.target_username = target_username.replace(/\r\n/g, '');
  ...
  formatTAGS(tagstring) {
    ...
    if (tagObject.badges) tagObject.badges = this.formatBADGES(tagObject.badges);
  ...
  formatTagName(tag) {
    ...
    if(tag.includes('-')) tag = tag.replace(/-/g, '_');
  ...
  formatTagVal(val) {
    ...
    if(!val) return null;
    if(val.match(/^[0-9]+$/) !== null) return +val;
    if (val.includes('\s')) val = val.replace(/\\s/g, ' ');
